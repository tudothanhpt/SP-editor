...
def Test1():
    ...
    merged_df = pd.merge(df_loadcalculationCase,df_SD, how='left', left_on='sds', right_on='SDName')
    merged_df = pd.merge(merged_df, df_designforce,  how='left', left_on=['tier','pier'], right_on = ['Tier','Pier'])
    ...
    for index, row in merged_df.iterrows():
        cover = row['barCover']
        bar_area = row['barArea']  # Assuming barArea is not in the DataFrame
