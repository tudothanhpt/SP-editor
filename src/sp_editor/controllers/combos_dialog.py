...
from sp_editor.crud.cr_load_combo import (
    TB_COMBOSELECTION,
    TB_CS_HEADER_SELECTEDCOMBO,
    TB_CS_HEADER_ORICOMBO,
)
...
class Combo_Dialog(qtw.QDialog, Ui_combosSelection_dialog):
    ...
    def load_DB(self, engine: Engine) -> None:
        ...
        self.lview_selectedCombos_list: list[str] = (
            self.tmpDF.iloc[:, 1].dropna().tolist()
        )

        self.lview_combos_model: ListModel = ListModel(
            self.lview_combos_list, self.ORDER_DICT
        )
        self.lview_selectedCombos_model: ListModel = ListModel(
            self.lview_selectedCombos_list, self.ORDER_DICT
        )

        self.lview_combos.setModel(self.lview_combos_model)
        self.lview_selectedCombos.setModel(self.lview_selectedCombos_model)

        self.lview_combos_listIndices: List[Tuple[str, int]] = (
            self.lview_combos.model().get_items_with_indices()
        )
        self.lview_selectedCombos_listIndices: List[Tuple[str, int]] = (
            self.lview_selectedCombos.model().get_items_with_indices()
        )
    ...
    def move_left(self):
        ...
        selected_items = [
            (
                self.lview_selectedCombos.model().data(index, qtc.Qt.DisplayRole),
                index.row(),
            )
            for index in sorted(
    ...
    def on_ok_clicked(self):
        ...
        self.tmpDF.to_sql(
            TB_COMBOSELECTION, con=self.engine, if_exists="replace", index=False
        )

        qtw.QMessageBox().information(
            self, "Notifications", f"{totalSelectedCombos}/{total} selected!"
        )
        self.close()
...
def main():
    ...
    engine_temppath = (
        r"C:\Users\abui\Desktop\Git\repo\SP-editor\tests\TestBM\DEMONO1.spe"
    )
